            ...
            switch (buffer[i]) {
                ...
                default:
                    if (optionType != PACIFY_TYPE_NONE) {
                        printf("[PACIFY] Error parsing config file on char %u\n", i);
                        exit(1);
                    }
                    ...
                    switch (buffer[i]) {
                        case 'i':
                            ...
                            pacify_option_int_set(optionName, atoi(optionValue));
                            break;
                        case 'd':
                            ...
                            pacify_option_double_set(optionName, strtod(optionValue, NULL));
                            break;
                        case 't':
                            ...
                            break;
                        default:
                            printf("[Pacify] Error parsing config file on char %u; Not a valid variable name.\n", i);
                            exit(1);
                            break;
                    }
    ...
    for (i = 0; i < pacifyCurrentState->optionAmount; i++) {
            switch (pacifyCurrentState->options[i].type) {
                case PACIFY_TYPE_INT:
                    printf("[Pacify] Int: ");
                    printf("%s %d\n", pacifyCurrentState->options[i].name, pacifyCurrentState->options[i].integer);
                    break;
                case PACIFY_TYPE_DOUBLE:
                    printf("[Pacify] Double: ");
                    printf("%s %f\n", pacifyCurrentState->options[i].name, pacifyCurrentState->options[i].decimal);
                    break;
                case PACIFY_TYPE_TEXT:
                    printf("[Pacify] Text: ");
                    printf("%s %s\n", pacifyCurrentState->options[i].name, pacifyCurrentState->options[i].text);
                    break;
...
pacify_option_int_set(char *optionName, int optionValue)
{
    ...
    pacifyCurrentState = realloc(pacifyCurrentState, sizeof(pacifyState) + sizeof(option) * pacifyCurrentState->optionAmount++);
    pacifyCurrentState->options[pacifyCurrentState->optionAmount - 1].type = PACIFY_TYPE_INT;
    pacifyCurrentState->options[pacifyCurrentState->optionAmount - 1].name = malloc(sizeof(char) * (strlen(optionName) + 1));
    strcpy(pacifyCurrentState->options[pacifyCurrentState->optionAmount - 1].name, optionName);
    pacifyCurrentState->options[pacifyCurrentState->optionAmount - 1].integer = optionValue;
}
...
pacify_option_double_set(char *optionName, double optionValue)
{
    ...
    pacifyCurrentState = realloc(pacifyCurrentState, sizeof(pacifyState) + sizeof(option) * pacifyCurrentState->optionAmount++);
    pacifyCurrentState->options[pacifyCurrentState->optionAmount - 1].type = PACIFY_TYPE_DOUBLE;
    pacifyCurrentState->options[pacifyCurrentState->optionAmount - 1].name = malloc(sizeof(char) * (strlen(optionName) + 1));
    strcpy(pacifyCurrentState->options[pacifyCurrentState->optionAmount - 1].name, optionName);
    pacifyCurrentState->options[pacifyCurrentState->optionAmount - 1].decimal = optionValue;
}
...
pacify_option_text_set(char *optionName, char *optionValue)
{
    ...
    pacifyCurrentState = realloc(pacifyCurrentState, sizeof(pacifyState) + sizeof(option) * pacifyCurrentState->optionAmount++);
    pacifyCurrentState->options[pacifyCurrentState->optionAmount - 1].type = PACIFY_TYPE_TEXT;
    pacifyCurrentState->options[pacifyCurrentState->optionAmount - 1].name = malloc(sizeof(char) * (strlen(optionName) + 1));
    strcpy(pacifyCurrentState->options[pacifyCurrentState->optionAmount - 1].name, optionName);
    pacifyCurrentState->options[pacifyCurrentState->optionAmount - 1].text = malloc(sizeof(char) * (strlen(optionValue) + 1));
    strcpy(pacifyCurrentState->options[pacifyCurrentState->optionAmount - 1].text, optionValue);
}
